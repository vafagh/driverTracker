                    ...
                    @if (strstr(URL::full(),'delivery_date') && strstr(URL::full(),'shift'))
                        <div class="batchUpdate d-none d-sm-inline btn-group" role="group">
                            <button id="filterlist" type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <i title="Filter" class="material-icons">play_for_work</i> Set All
                            </button>
                            <div class="dropdown-menu" aria-labelledby="filterlist">
                                <a class="dropdown-item" href="{{strtolower($op2)}}/massUpdate{{str_replace(URL::current(),'',URL::full())}}&amp;newShift=Morning&amp;newDelivery_date={{\Carbon\Carbon::today()->toDateString()}}">Morning</a>
                                <a class="dropdown-item" href="{{strtolower($op2)}}/massUpdate{{str_replace(URL::current(),'',URL::full())}}&amp;newShift=Evening&amp;newDelivery_date={{\Carbon\Carbon::today()->toDateString()}}">Evening</a>
                            </div>
                        </div>
                    @endif
                @endif
                    ...
                    @if ($collection->count()>0)
                        <div class=" sort btn-group" role="group">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="sortList">
                                                <a class="dropdown-item" href="?{{str_replace(URL::current(),'',str_replace('sortby=','oldSortby=',URL::full()))}}&amp;sortby=invoice_number">#</a>
                                                ...
                                                <a class="dropdown-item" href="?{{str_replace(URL::current(),'',URL::full())}}&amp;sortby=updated_at">Update</a>
                                                <a class="dropdown-item" href="?{{str_replace(URL::current(),'',URL::full())}}&amp;sortby=status">Status</a>
                                                <a class="dropdown-item" href="?{{str_replace(URL::current(),'',URL::full())}}&amp;sortby=user_id">User</a>
                                            </div>
                                        </div>
                    @endif
                @endif
